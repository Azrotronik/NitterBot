...
while(1==1)
	...
	#do nothing, ignore PMs 
        ...
        searchterm=line[line.index(':p#')+3..line.length].chomp
        if sanitizeinput(searchterm)
	        if `curl -Ns "https://nitter.cc/search?f=tweets\&q=%23#{searchterm}" | grep -Eo 'No items found'`.include?"No items found" or not(`curl -Ns "https://nitter.cc/search?f=tweets\&q%23=#{searchterm}" | grep -Eo "src=\\"/pic/media.+(jpg|png)" | sed -E "s/src=\\"//" | shuf -n1`=~/jpg|png/)
   			    ssl.puts "PRIVMSG #{channel} :Sorry, got nothing!"
			    next
	        end
